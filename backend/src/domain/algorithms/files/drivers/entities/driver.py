...
class Driver:
    ...
    def valorar_descanso(self, viaje):
        if (self.jornada <= timedelta(hours=5)) and (self.descanso_inicio is None):
    ...
    def esta_descansando(self, viaje):
        ...
        if self.descanso_inicio is None:
            return False
    ...
    def viaje_disponible(self, viaje):
        if not (self.esta_descansando(viaje)):
            ...
            if self.jornada >= viaje.tiempo_viaje:
                ...
                if (
                    (self.lugar is None)
                    or (self.lugar == viaje.estacion_salida)
                    or (self.puede_viaje_vacío(viaje))
                ):
                    ...
                    if (self.hora_final_anterior is None) or (
    ...
    def puede_viaje_vacío(self, viaje):
        ...
        if self.tiempo_ultimo_viaje is None:
            return False
    ...
    def asginar_viaje(self, viaje):
        if self.hora_inicio_jornada is None:
        ...
        tiempo_transcurrido = (
            viaje.hora_inicio - self.hora_final_anterior
        ) + viaje.tiempo_viaje
