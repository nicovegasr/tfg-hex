...
def start(*arguments):
    ...
    file.columns = ["Dia", "Linea",
                       "Fecha_Salida", "Fecha_Llegada", "Direccion"]
    file['Fecha_Salida'] = pd.to_datetime(
        file['Fecha_Salida'], format='%Y-%m-%d %H:%M:%S')
    file['Fecha_Llegada'] = pd.to_datetime(
        file['Fecha_Llegada'], format='%Y-%m-%d %H:%M:%S')
    ...
    for date_value in range(len(departure_dates)):
        if departure_dates[date_value] > arrival_dates[date_value]:
            raise Exception(f"La fecha de salida no puede ser mayor que la de llegada, porfavor revise las fechas en la fila con valor fecha de salida: {departure_dates[date_value]}.")
