...
class DefaultCheck:
    ...
    def __init__(
        self,
        filenames: list[str],
        content_files: list[pd.DataFrame],
    ...
    def start(self) -> None:
        ...
        for current_content_file in range(len(self.content_files)):
            ...
            try:
                ...
                raise Exception(
                    f"En el fichero {self.filenames[current_content_file]}: {str(error)}"
                )
    ...
    def check_n_cols(self, n_cols, file):
        if not file.shape[1] == int(n_cols):
            raise Exception("El número de columnas del fichero es incorreccto.")
    ...
    def types(self, types, file):
        ...
        if file.shape[1] != len(types):
            raise Exception(
                f"En el fichero de configuración, no ha indicado el número correcto de types,\
            ...
            if tipo == "str":
                for fila, valor in enumerate(columna):
                    try:
                        ...
                        raise Exception(
                            f"En la columna {column + 1}, fila {fila + 1}, el valor {valor} no es del tipo 'str'"
                        )
            elif tipo == "int":
                for fila, valor in enumerate(columna):
                    if math.isnan(valor):
                    ...
                    try:
                        ...
                        raise Exception(
                            f"En la columna {column + 1}, fila {fila + 1}, el valor {valor} no es del tipo 'int'"
                        )
            elif tipo == "float":
                for fila, valor in enumerate(columna):
                    if math.isnan(valor):
                    ...
                    try:
                        ...
                        raise Exception(
                            f"En la columna {column + 1}, fila {fila + 1}, el valor {valor} no es del tipo 'float'"
                        )
            elif tipo == "Datetime":
                for fila, valor in enumerate(columna):
                    try:
                        ...
                        raise Exception(
                            f"En la columna {column + 1}, fila {fila + 1}, el valor {valor} no es una fecha y hora válida"
                        )
            else:
                raise Exception(
                    f"En la columna {column + 1}, el tipo '{tipo}' es inválido. {type(valor)}"
                )
