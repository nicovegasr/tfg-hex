...
def add_download_button():
  return (html.Button('Descargar', id={
...
def get_visualizer(files, algorithm_name):
  ...
  if importlib.util.find_spec(f"src.domain.entitities.visualizer.{algorithm_name}"):
        algorithm_visualizer_module = importlib.import_module(f"src.domain.entitities.visualizer.{algorithm_name}")
        algorithm_visualizer = algorithm_visualizer_module.visualizer(files_in_dataframe)
  else:
